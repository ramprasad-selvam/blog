...
import { 
  Maximize, Weight, Ruler, Thermometer, 
  Droplet, HardDrive, Copy, Scale, ArrowRightLeft,
  ChevronRight, Gauge, Navigation, Info, Check
} from "lucide-react";
...
export default function UniversalConverter() {
  ...
  const handleConvert = (cat: Category, unit: string, val: string) => {
    ...
    if (cat === "area") {
      const f: any = { acre: 43560, cent: 435.6, sqft: 1, sqm: 10.7639, sqyard: 9, hectare: 107639 };
      const base = v * f[unit];
      ...
      const f: any = { m: 1, ft: 0.3048, in: 0.0254, cm: 0.01, mm: 0.001, km: 1000, mile: 1609.34 };
      const base = v * f[unit];
      ...
      const f: any = { kg: 1, lbs: 0.453592, oz: 0.0283495, g: 0.001, ton: 1000, mg: 0.000001 };
      const base = v * f[unit];
      ...
      const f: any = { l: 1, gal: 3.78541, ml: 0.001, cf: 28.3168, cup: 0.236588, pt: 0.473176 };
      const base = v * f[unit];
      ...
      const f: any = { kmh: 1, mph: 1.60934, ms: 3.6, knot: 1.852 };
      const base = v * f[unit];
      ...
      const f: any = { bar: 1, psi: 0.0689476, pascal: 0.00001, atm: 1.01325 };
      const base = v * f[unit];
      setPressure({ bar: base.toFixed(3), psi: (base / f.psi).toFixed(2), pascal: (base / f.pascal).toFixed(0), atm: (base / f.atm).toFixed(3) });
    } else if (cat === "digital") {
      const f: any = { gb: 1024, mb: 1, tb: 1048576, bit: 0.000000119209, kb: 0.0009765625 };
      const base = v * f[unit];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-3 bg-zinc-900/20 border border-white/5 rounded-[3.5rem] p-8 md:p-12">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {activeTab === "area" && Object.entries(area).map(([u, v]) => <Field key={u} label={u} val={v} on={val => handleConvert('area', u, val)} id={u} copy={copy} copied={copied} />)}
            {activeTab === "length" && Object.entries(length).map(([u, v]) => <Field key={u} label={u} val={v} on={val => handleConvert('length', u, val)} id={u} copy={copy} copied={copied} />)}
            {activeTab === "mass" && Object.entries(mass).map(([u, v]) => <Field key={u} label={u} val={v} on={val => handleConvert('mass', u, val)} id={u} copy={copy} copied={copied} />)}
            {activeTab === "volume" && Object.entries(volume).map(([u, v]) => <Field key={u} label={u} val={v} on={val => handleConvert('volume', u, val)} id={u} copy={copy} copied={copied} />)}
            {activeTab === "speed" && Object.entries(speed).map(([u, v]) => <Field key={u} label={u} val={v} on={val => handleConvert('speed', u, val)} id={u} copy={copy} copied={copied} />)}
            {activeTab === "pressure" && Object.entries(pressure).map(([u, v]) => <Field key={u} label={u} val={v} on={val => handleConvert('pressure', u, val)} id={u} copy={copy} copied={copied} />)}
            {activeTab === "digital" && Object.entries(digital).map(([u, v]) => <Field key={u} label={u} val={v} on={val => handleConvert('digital', u, val)} id={u} copy={copy} copied={copied} />)}
            {activeTab === "temp" && Object.entries(temp).map(([u, v]) => <Field key={u} label={u} val={v} on={val => handleTemp(u, val)} id={u} copy={copy} copied={copied} />)}
          </div>
        </div>

        {/* DYNAMIC REFERENCE CARD (Sidebar) */}
        <div className="lg:col-span-1">
          ...
          <div className="mt-6 p-8 rounded-[3rem] bg-zinc-900/40 border border-white/5">
             <h4 className="text-[10px] font-black text-zinc-600 uppercase tracking-widest mb-2 text-center">Base Logic</h4>
             <p className="text-[11px] text-zinc-500 font-bold italic text-center">Calculated using floating point IEEE-754 precision.</p>
          </div>
        </div>
...
function Field({ label, val, on, id, copy, copied }: any) {
  return (
...
function ReferenceCard({ title, notes, items }: { title: string; notes: string; items: string[] }) {
  return (
    <div className="p-8 rounded-[3rem] bg-blue-600 text-white shadow-2xl shadow-blue-600/20 animate-in slide-in-from-right-4 duration-500">
